...
const richContentCheckbox = document.getElementById('richContentCheckbox');
const accentLinkCheckbox = document.getElementById('accentLinkCheckbox');
...
function getHtmlContent(includeStyles = true) {
  ...
  const contentHtml = sections.content.innerHTML.trim() || (richContentCheckbox.checked ? 
    `<h2>Welcome</h2><img src="placeholder.jpg" alt="Placeholder"><ul><li>Item 1</li><li>Item 2</li></ul>` : 
    `<p>Hello World</p>`);
  const accentContent = accentLinkCheckbox.checked ? `<a href="#">↥</a>` : '';
  return `
...
<body>
    ...
    <div class="page-container">
        <!-- <header> for site title or logo (edit the <h1> text) -->
        <header>
            <h1>Sample Heading</h1>
        </header>
        <!-- <nav> for navigation links (edit or add <a> tags) -->
        <nav>
            <a href="#">Home</a>
            <a href="#">About</a>
        </nav>
        <!-- <main> for main content. Replace or add text, images, lists, etc. -->
        <main>${contentHtml}</main>
        <!-- <footer> for copyright or contact info (edit the text) -->
        <footer>
            <p>© 2026 Your Name</p>
        </footer>
        <!-- Accent: Decorative circle. Remove this <div>, change its shape (edit CSS), or add content (e.g., <a> or <img>) -->
        <div class="accent">${accentContent}</div>
    </div>
</body>
</html>
  `;
